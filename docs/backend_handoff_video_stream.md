...
部署时视频流后端建议使用 1 个 Gunicorn worker + 多线程，例如 `WORKERS=1`、`THREADS=16`、`TIMEOUT=300`。这样同一 `stream_id` 在同一进程内只保留一个 `CameraWorker`，多个浏览器访问共享 `latest_frame`，避免重复创建 RTSP 连接。
