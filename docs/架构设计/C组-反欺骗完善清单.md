...
> **负责人**：王梓铭（C 组）  
> **对照文档**：`docs/课程要求/4_小学期任务清单.pdf` → 人脸识别可选项「防御静态图片、视频、AI 换脸等欺骗认证（8 分）」  
> **编写日期**：2026-07-10  
> **最后更新**：2026-07-12  
> **当前分支**：`dev`  
> **状态**：必改项（§二）已全部完成，本文档保留为验收记录
...
| 攻击告警带截图 | `liveness.py` → `_persist_alert(frame=...)` | 告警中心可回放截图 |
| 多帧活体注册 | `FaceRegisterView` + `FamilyRegister.vue` | 静态图多次提交会被拒绝 |
| 活体 API | `POST /api/face/liveness/` | Swagger 可调用 |
| 监控页活体状态 | `HomeMonitor.vue` + `/api/video/status` | 显示 passed / attack / insufficient 标签 |
| WebRTC 上 AI 标注 | `FaceOverlay.vue` + `/api/video/presence/` | 监控页可见检测框（不依赖 MJPEG 预览） |
| 告警类型 | `backend/apps/alerts/models.py` | `FACE_SPOOF` / `FACE_REPLAY` / `FACE_DEEPFAKE` |
| 告警中心筛选 | `frontend/src/views/AlertCenter.vue` | 按攻击类型筛选、处置、忽略 |
| 单元测试 | `backend/apps/face/tests/test_services.py` | 含 snapshot、活体序列测试 |

### 可选加分项（未实现，不阻塞答辩）

- 主动动作挑战（眨眼/转头）
- Aurora Guard 光反射方案
- Silent-Face-Anti-Spoofing 等深度学习模型替换启发式
- `FACE_AUTH_FAILED` 独立检测逻辑

---

## 二、必改项（已完成，2026-07-12）
...
| 事项 | 负责 | 状态 |
|------|------|------|
| WebRTC 流叠加 AI 框 | A 组 `FaceOverlay.vue` | ✅ 已完成 |
| 告警中心筛选/忽略 UI | A 组 | ✅ 已完成 |
| OpenSpec 联调任务 4.1/4.2 | 全员 | ✅ 已验收并归档 |

C 组活体检测在 **RTSP → process_frame()** 链路生效；前端通过 **overlay API** 展示结果，不依赖 MJPEG 预览。
...
- [x] 静态照片对摄像头 → 10 秒内产生 `FACE_SPOOF` 告警
- [x] 该告警可在告警中心筛选、处置，且 **回放有截图**
- [x] 注册时提交同一张图多次 → **拒绝注册**
- [x] 真人面对摄像头连续采集 → **注册成功**
- [x] Swagger 存在 `POST /api/face/liveness/` 且文档完整
- [x] 监控页显示活体状态（正常 / 检测中 / 疑似欺骗）
- [x] `python manage.py test apps.face` 全部通过
- [x] 推 `feature/face` → GitHub Actions CI 绿
...
| `openspec/changes/archive/2026-07-12-ai-video-integration/tasks.md` | OpenSpec 任务（§2 人脸识别，已归档） |
