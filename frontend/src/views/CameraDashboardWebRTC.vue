...
import { CAMERA_STREAMS, videoFeedPath, webrtcPreviewUrl } from '@/constants/streams'

const cameras = CAMERA_STREAMS.map((stream) => ({
  id: stream.id,
  name: `${stream.label}摄像头`,
  position: `推流通道 stream/${stream.id}`,
  webrtcUrl: webrtcPreviewUrl(stream.id),
  mjpegUrl: videoFeedPath(stream.id),
}))
